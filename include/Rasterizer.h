...
//remake of Rasterizer but this time only for lightspace buffers
class RasterizerDepth
{
	public:
		RasterizerDepth(){}
		~RasterizerDepth(){}
	...
	static void DrawTriangle(Point2D& p1, Point2D& p2, Point2D& p3, int& w, int& h, float* depthBuffer)
		{
			// first sort the points in descending order a/c y coordinate
			SortY(p1,p2,p3);
			...
			if(p1.y == p2.y)  // the two upper points are at same y level
			{
				...
				Point2D e1[] = {p1,p3};
				Point2D e2[] = {p2,p3};
				Interpolate(e1,e2,w, h, depthBuffer);
			}
			else if (p2.y == p3.y) // the two lower points are at same y level
			{
				//std::cout << "lower base";
		...
		static void Interpolate(Point2D* e1, Point2D* e2, int& w, int& h, float* depthBuffer)
		{
			// function receives flat top or flat bottom triangle
			// make sure that e1 is left edge
			if(e1[0].x > e2[0].x or e1[1].x > e2[1].x) // if e1 is on right, swap the edges
